...
export default function ManageBuses() {
  ...
  const [form, setForm] = useState({ busNumber: "", type: "AC", capacity: 32 });
  const [loading, setLoading] = useState(true);
  const [msg, setMsg] = useState("");
  const [editingId, setEditingId] = useState(null);

  // ... (Keep all your functions: fetchBuses, onChange, onSubmit, onEdit, onDelete) ...
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ busNumber: "", type: "AC", capacity: 32 });
      setEditingId(null);
      await fetchBuses();
    } catch (err) {
  ...
  const onEdit = (b) => {
    setEditingId(b.id);
    setForm({ busNumber: b.busNumber, type: b.type, capacity: b.capacity });
  ...
  return (
    <div>
      <h3 className="mb-4 fw-bold">Manage Buses</h3>
      {msg && (
      ...
      <div className="card card-elev p-4 mb-4">
        <form onSubmit={onSubmit} className="row g-3">
          ...
          <div className="col-md-6">
            ...
            <input type="number" className="form-control" name="capacity" value={form.capacity} onChange={onChange} required />
          </div>
          <div className="col-12">
            <button className="btn btn-brand">{editingId ? "Update Bus" : "Add Bus"}</button>
            {editingId && (
      ...
      <div className="card card-elev p-4">
        <h5 className="mb-3">Existing Buses</h5>
        {loading ? (
          ...
          <div className="list-group">
            {buses.map(b => (
              <div key={b.id} className="list-group-item d-flex flex-wrap justify-content-between align-items-center">
                <div>
                  <strong>{b.busNumber}</strong> - {b.type} ({b.capacity} seats)
                </div>
